  ...
  <BaseDrawer :open="isOpen" @close="closePopup">
    <div class="w-full h-full flex flex-col overflow-auto -translate-y-12">
      <template v-if="product.media?.length">
        <div class="w-full h-65 rounded-t relative">
          <div class="absolute w-full top-0 h-65 border-b-1 border-base-300 overflow-hidden flex flex-col justify-center rounded-t">
            <DiagonalPattern class="scale-165 opacity-60 text-warning-content/80"
                             :establishment="establishment ?? 'restaurant'"/>
          </div>

          <Splide class="w-full h-65 rounded-t" :options="{
                    perPage: 1,
                    perMove: 1,
                    rewind: false,
                    rewindByDrag: false,
            ...
            <SplideSlide v-for="(m, index) in media" :key="m.id">
              <img class="w-full h-65 object-cover object-center rounded-t border-none"
                   :src="m.url" alt=""
      ...
      <div class="w-full text-warning-content/80 bg-warning/10 opacity-80 border-none select-none rounded-none py-3 px-2 font-semibold text-md text-start"
      ...
      <div class="flex-1">
        ...
        <div class="flex flex-col gap-1 px-6 py-1 bg-base-200/50">
          <!-- Nutrition and timing info -->
          <div class="flex gap-4" v-if="product.preparation_time || product.calories">
            <div v-if="product.preparation_time" class="flex items-center gap-2">
              <Timer class="w-5 h-5 text-gray-600" />
              <span class="font-semibold">{{ i18n.t('badges.time', { minutes: product.preparation_time }) }}</span>
            </div>

            <div v-if="product.calories" class="flex items-center gap-2">
              <Flame class="w-5 h-5 text-gray-600" />
              <span class="font-semibold">{{ i18n.t('badges.calories', { calories: product.calories }) }}</span>
            </div>
          </div>
          ...
          <!-- Price and Variants -->
          <div class="mb-4">
            <h4 class="font-semibold text-lg mb-3">{{ i18n.t('Variants') }}</h4>

            <!-- Base price -->
            <div class="flex justify-between items-center py-2 px-3 bg-base-200/50 rounded mb-2">
              <div class="flex items-center gap-2">
                <span class="font-medium">{{ product.weight }} {{ weightUnitFormatted(product.weight_unit ?? '') }}</span>
              </div>
            ...
            <div v-if="product.variants?.length" class="space-y-2">
              <div v-for="variant in product.variants" :key="variant.id"
                   class="flex justify-between items-center py-2 px-3 bg-base-200/50 rounded">
                <div class="flex items-center gap-2">
                  <span class="font-medium">{{ variant.weight }} {{ weightUnitFormatted(variant.weight_unit ?? '') }}</span>
                </div>
        ...
        <div class="px-4">
          ...
          <div class="mb-6">
            ...
            <div v-if="allergens.length > 0" class="mb-4">
              <h4 class="font-semibold text-lg mb-2 text-orange-600">{{ i18n.t('badges.allergens') }}</h4>
